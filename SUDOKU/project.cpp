...
string itos(int i){
	...
	if(i>0){ 
	    ...
	    temp="";
	    for(unsigned int i=0;i<s.length();i++){ temp=temp+s[s.length()-1-i]; } 
	}
	else if(i==0){ temp="0"; }
	else if(i<0){ 
		int m=-i;
...
class griddata{
    ...
    griddata(){ type = 'u'; num = 0; return; }
    
    griddata operator=(griddata p){ type=p.type; num=p.num; return*this;}
};
...
void clickfunction(int x,int y, griddata tempdata[9][9], Text grid[9][9], int& moves, Text& movenum, int w6){
   if(!((x<0||x>8) || (y<0 || y>8))){
       ...
       if(tempdata[x][y].type=='u'){
           ...
           if(x1>=0 && x1<9 && y1==0){
			  string s=itos(x1+1);
              grid[x][y].reset(470+x*20,370+y*20,s); tempdata[x][y].num=x1+1;} 
           else{ grid[x][y].reset(470+x*20,370+y*20,""); tempdata[x][y].num=0; }
           for(int i=0;i<9;i++){ 
...
void complgame(griddata tempdata[9][9], griddata soldata[9][9], griddata newdata[9][9], bool& next, int h, int w4, bool& a, int moves, int hints, int& hscore, int& level){
	...
	for(int i=0;i<9; i++){
		for(int j=0;j<9;j++){
			if(tempdata[i][j].num==soldata[i][j].num){ val=true; }
			else if(tempdata[i][j].num!=soldata[i][j].num){ val=false; break;}
		} if(val==false){ break; }
     }
	if(val==true){ 
		   Text complete(200,700,"CONGRATULATIONS! GAME COMPLETE...");
		   ...
		   if(level==1){ 
				...
				for(int i=0;i<9;i++){
	              for(int j=0;j<9;j++){ onpsol2<<newdata[i][j].type<<" "<<newdata[i][j].num<<" ";} cout<<endl; } onpsol2<<level<<endl<<hscore;
			  }
			else if(level==3){
				ofstream onpsol3("l3",ios::out);
				for(int i=0;i<9;i++){
	              for(int j=0;j<9;j++){ onpsol3<<newdata[i][j].type<<" "<<newdata[i][j].num<<" ";} cout<<endl; } onpsol3<<level<<endl<<hscore;
			  }
		   }
		   Text your_sc(250,650,itos(score));
		   Rectangle newBox(700,650,textWidth("NEXT LEVEL")+5,h+5); int w_newBox=(textWidth("QUIT GAME")+5)/2;
...
void Sudoku(griddata newdata[9][9],griddata soldata[9][9],griddata userdata[9][9], int h, bool& a, int w1, bool& next, int& level, int& hscore){
   ...
   Rectangle array[9][9]; Rectangle fixednum[9][9];
   for(int i=0;i<9;i++){
	   for(int j=0;j<9;j++){
		   fixednum[i][j].reset(470+i*20,370+j*20,20,20);
	   }
...
void fill_solver(int x, int y, Text grid[9][9], Rectangle fixednum[9][9],int board[9][9]){
	if(!((x<0||x>8) || (y<0 || y>8))){
	   string s;
	   int k=numbergrid(); int x1=(k/65536-460)/20; int y1= ((k % 65536)-790)/20;
       if(x1>=0 && x1<9 && y1==0){
...
void Sudoku_solver(bool& quit_solver, int h,int w1){
	 ...
	 while(getClick()){ 
	   ...
	   if(position/65536>700-w4 && position/65536<700+w4 && (position % 65536)>695-h && (position % 65536)<705+h){
			quit_solver=true; break; 
	         ...
	         for(int i=0;i<9;i++){ for(int j=0;j<9;j++){ out1<<board[j][i]<<" "; } out1<<endl; }
	         out1.close();
			 cout<<"slve"<<endl;
			 if(invalid==true){ unsolvable.reset(200,700,"INVALID INPUT. SUDOKU UNSOLVABLE!!!"); }
			 if(invalid==false){	 unsolvable.reset(200,700,"SUDOKU SOLVED!!!");
...
main_program{
   ...
   Rectangle Box1(100,100,textWidth("NEW GAME")+5,textHeight()+5); int w1=(textWidth("NEW GAME")+5)/2; int h=(textHeight()+5)/2;
   Text txtBox1(100,100,"NEW GAME"); bool a=true; bool next=false; int level; int hscore;
   ...
   for(int i=0;i<9;i++){
	   for(int j=0;j<9;j++){ inp1>>newdata[i][j].type>>newdata[i][j].num;} }  inp1>>level; inp1>>hscore;
   ifstream inp2("soldata_l1",ios::in);
   for(int i=0;i<9;i++){
	   for(int j=0;j<9;j++){ inp2>>soldata[i][j].type>>soldata[i][j].num;} }
   ...
   if(a==false){ wait(0); }
	   ...
	   for(int i=0;i<9;i++){
	        for(int j=0;j<9;j++){ inp1_2>>newdata[i][j].type>>newdata[i][j].num;} }  inp1_2>>level; inp1_2>>hscore;
       ifstream inp2_2("soldata_l2",ios::in);
       for(int i=0;i<9;i++){
	        for(int j=0;j<9;j++){ inp2_2>>soldata[i][j].type>>soldata[i][j].num;} }
	   Sudoku(newdata, soldata, userdata,h, a, w1, next, level, hscore);	
	   if(a==false){ wait(0); }
	   else if(next==true){
		   a=true; next=false;   
	       ...
	       for(int i=0;i<9;i++){
	            for(int j=0;j<9;j++){ inp1_3>>newdata[i][j].type>>newdata[i][j].num;} } inp1_3>>level; inp1_3>>hscore;
           ifstream inp2_3("soldata_l3",ios::in);
           for(int i=0;i<9;i++){
	            for(int j=0;j<9;j++){ inp2_3>>soldata[i][j].type>>soldata[i][j].num;} }
	       Sudoku(newdata, soldata, userdata,h, a, w1, next,level, hscore);	
	       if(a==false){ wait(0); }
           else if(next==true){wait(0);  } }
   } } 
	   ...
	   bool quit_solver=false;
	    Rectangle Box1(100,100,textWidth("NEW INPUT")+5,textHeight()+5); int w1=(textWidth("NEW INPUT")+5)/2; int h=(textHeight()+5)/2;
        Text txtBox1(100,100,"NEW INPUT");
	   while(getClick()){ 
        int pos=getClick(); int x=pos/65536; int y=pos%65536;
        if(x>100-w1 && x<100+w1 && y>100-h && y<100+h){
			Sudoku_solver(quit_solver,h,w1);
	        if(quit_solver==true){ break; }
           }
	   }
