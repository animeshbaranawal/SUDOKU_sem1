...
namespace simplecpp{
	...
	public:
		...
		QPointF prod(Position p) const{
			QPointF xp;
			xp.setX(a11*p.x+a12*p.y+a13 + 0.5);  // round, not truncate.
			xp.setY(a21*p.x+a22*p.y+a23 + 0.5);
			return xp;
		}
		...
		void compute_res_vertex(QPointF* xpt, vector<Position> vertex, int count) const{
			for(int i=0; i<count; i++)
				xpt[i] = prod(vertex[i]);
		}

		void compute_res_vertex(QPointF* xpt, Position *vertex, int count) const{
			for(int i=0; i<count; i++)
				xpt[i] = prod(vertex[i]);
		}
