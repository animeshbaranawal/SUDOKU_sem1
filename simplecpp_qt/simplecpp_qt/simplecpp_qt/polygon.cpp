...
namespace simplecpp{
	...
	Polygon::Polygon(const double x, const double y, const double points[][2], int no, Composite *owner) : Sprite(x,y,owner){
		init(x, y, points,no, owner);
	}

	void Polygon::reset(const double x, const double y, const double points[][2], int no, Composite *owner){
		vertex.clear();
	...
	void Polygon::init(const double x, const double y, const double points[][2], int no, Composite *owner){
	...
	void Polygon::paint(Pose *p){
		if(visible && vertex.size()>0){
			QPointF* xpt_vertex = new QPointF[vertex.size()];
			if(p){
				Pose r = Pose(*p,pose);
